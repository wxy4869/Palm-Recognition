...
if K == 1:
    LOG = Logger('log/sum/%s_k=%d_%s_size=%d_q=%d_t=%d_e=%f_mu=%f_tp=%f_tn=%f_lambda=%f_knn=%d.log' % (DATASET, K, FEATURES[0], SIZE_TRAIN, Q, T, E, MU, T_P, T_N, LAMBDA, param.N_NEIGHBORS), level='info')
else:
    LOG = Logger('log/sum/%s_k=%d_multi_size=%d_q=%d_t=%d_e=%f_mu=%f_tp=%f_tn=%f_lambda=%f_knn=%d.log' % (DATASET, K, SIZE_TRAIN, Q, T, E, MU, T_P, T_N, LAMBDA, param.N_NEIGHBORS), level='info')
...
def cal_similarity(x1, x2, W):
    ...
    similarity = torch.sum(similarity, 1) / K
    return similarity
...
def multi(x1, x2, label):
    ...
    # torch.save(similarity, 'result/similarity/sum_before.pt')
    ...
    for t in range(T):
        ...
        if abs(J - J_new) < E:
            # torch.save(similarity, 'result/similarity/sum_tongji_after.pt')
            # quit()
            return W
        J = J_new
...
def val_verify(df, feature, i, W, best_threshold):
    with torch.no_grad():
        ...
        # 验证
        similarity = cal_similarity(x1, x2, W).cpu()
...
def test_verify(df, i, scaler, pca, W, best_model):
    with torch.no_grad():
        ...
        if K == 1:
            np.save('result/sum/%s_k=%d_%s.npy' % (DATASET, K, FEATURES[0]), {'fpr': fpr, 'tpr': tpr, 'thresholds': thresholds})
        else:
            np.save('result/sum/%s_k=%d_multi.npy' % (DATASET, K), {'fpr': fpr, 'tpr': tpr, 'thresholds': thresholds})
